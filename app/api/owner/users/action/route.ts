...
export async function POST(req: Request) {
  try {
    ...
    if (action === "reset") { if (!email) return NextResponse.json({ error: "Email is required for password reset." }, { status: 400 }); const redirectTo = process.env.NEXT_PUBLIC_SITE_URL || process.env.NEXT_PUBLIC_APP_URL || "https://on-point-inspect-mvp.vercel.app/login"; const { error } = await admin.auth.resetPasswordForEmail(email, { redirectTo }); if (error) throw error; message = "Password reset email sent."; }
