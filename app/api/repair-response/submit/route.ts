  ...
  <body style="margin:0; padding:0; background:#f8fafc; font-family:Arial, Helvetica, sans-serif; color:#0f172a;">
    <div style="max-width:680px; margin:0 auto; padding:24px;">
      <div style="background:#ffffff; border:1px solid #cbd5e1; border-radius:14px; padding:24px;">
        ...
        <p style="font-size:16px; line-height:1.5; margin:0 0 8px 0;">The repair request for:</p>
        ...
        <p style="margin:0 0 24px 0;">
          <a href="${escapeHtml(trackedRepairRequestUrl)}" style="display:inline-block; background:#0f172a; color:#ffffff; padding:12px 18px; border-radius:10px; text-decoration:none; font-weight:700;">View Repair Request</a>
        </p>
...
The repair request for ${property} is ready to review and respond to.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    for (const recipient of recipients) {
      ...
      const responseUrl = `${appUrl}/repair-response/${share.token}`;
      ...
      const trackedResponseUrl = `${appUrl}/api/email-click?inspection_id=${encodeURIComponent(
        ...
        recipient.recipientType
      )}&recipient_email=${encodeURIComponent(recipient.email)}&target=${encodeURIComponent(responseUrl)}`;
