...
async function sendWebPush(subscription: any, payload: any) {
  ...
  const subject = process.env.VAPID_SUBJECT || "mailto:jeffshockey90@gmail.com";
...
export async function POST(req: Request) {
  try {
    ...
    const message = String(body.body || body.message || "New activity recorded.");
    const url = String(body.url || "/dashboard/owner");
    const eventType = String(body.eventType || body.event_type || "manual");
