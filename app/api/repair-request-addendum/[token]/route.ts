...
function buildSignatureBox(label: string, signature: any) {
  ...
  const signatureImage = cleanText(signature?.signature_image || signature?.signatureImage);
  ...
  if (!signedName && !signatureImage && !printedName) {
    return `
  ...
  return `
    <div class="signature-box signed">
      ...
      ${
        signatureImage
          ? `<img class="drawn-signature" src="${escapeHtml(signatureImage)}" alt="${escapeHtml(label)} signature" />`
...
function buildAddendumHtml(data: Awaited<ReturnType<typeof loadAddendumData>>) {
  ...
  const fullyExecuted = Boolean(
    (signatures.buyer?.signature || signatures.buyer?.signature_image) &&
      (signatures.seller?.signature || signatures.seller?.signature_image)
  );
  const documentId = cleanText(data.share.id || data.share.token || "");
...
<head>
  ...
  <style>
    * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    ...
    .drawn-signature { width: 100%; height: 58px; object-fit: contain; border-bottom: 1px solid #0f766e; background: #ecfeff; }
  ...
  <main class="shell">
    <section class="paper">
      <div class="brand">
        ...
        <div class="badge">${fullyExecuted ? "Fully Executed" : "Addendum"}</div>
      </div>
      ...
      <div class="summary">
        ...
        <div><span>Document ID</span><strong>${escapeHtml(documentId)}</strong></div>
        <div><span>Execution Status</span><strong>${fullyExecuted ? "Fully Executed" : "Pending Signature"}</strong></div>
      </div>
      ...
      <p class="footer-note">Electronically executed through On Point Inspect. Generated by On Point Inspect for repair negotiation review. Final terms should be confirmed by the parties and their real estate professionals.</p>
    </section>
  </main>
</body>
</html>`;
