...
import { houseMemory, houseRelationshipEngine } from "../../../../lib/ai";
import {
  getSessionUser,
  unauthorized,
  notFound,
...
export async function POST(req: Request) {
  try {
    ...
    if (findingsResult.error) {
      console.error("House relationships findings load error:", findingsResult.error);
    }

    if (equipmentResult.error) {
      console.error("House relationships equipment load error:", equipmentResult.error);
    }
    ...
    if (photosError) {
      console.error("House relationships photos load error:", photosError);
    }
    ...
    const result = houseRelationshipEngine.analyze({
      inspection,
      findings: normalizedFindings,
      equipment,
      houseMemory: memory,
    ...
    console.error("HOUSE RELATIONSHIPS ERROR:", error);
