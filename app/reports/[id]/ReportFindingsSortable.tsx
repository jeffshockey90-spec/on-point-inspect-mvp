  ...
  function handleDragEnd(
    ...
    setSections((items) => {
      const oldIndex = items.findIndex(
        (item) =>
          item.section === active.id
      );

      const newIndex = items.findIndex(
        (item) =>
          item.section === over.id
      );
          ...
          {allOptions.map(
            (option: string) => {
              ...
              return (
                <label
                  key={option}
                  className="flex w-full cursor-pointer items-center gap-2 rounded-lg px-2 py-1 text-sm text-slate-200 hover:bg-slate-800/80"
                >
                  <input
                    type={
                      checklist.type ===
