...
export default async function PricingPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[var(--fl-ground)] px-4 py-10 text-[var(--fl-text)] md:px-6 md:py-16">
      ...
      <div className="relative mx-auto max-w-5xl">
        <div className="text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-[var(--fl-muted)]">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2">
          <div className="rounded-2xl border border-[var(--fl-line)] bg-[var(--fl-surface)] p-8">
            <p className="text-sm font-semibold uppercase tracking-wide text-[var(--fl-muted)]">
              Free Trial
            </p>
            <p className="mt-4 text-5xl font-semibold text-[var(--fl-text)]">$0</p>
            <p className="mt-2 text-[var(--fl-muted)]">
              Your first {FREE_INSPECTIONS} inspections, on us
            </p>
            <p className="mt-6 text-sm leading-6 text-[var(--fl-muted)]">
            ...
            <Link
              href="/signup"
              className="mt-8 inline-flex w-full items-center justify-center rounded-xl border border-[var(--fl-line)] px-6 py-3 text-center font-semibold text-[var(--fl-text)] transition hover:bg-[var(--fl-raised)]"
            >
              Start Free
            </Link>
          </div>

          <div className="relative rounded-2xl border border-teal-500/70 bg-gradient-to-b from-teal-500/10 to-[var(--fl-surface)] p-8 shadow-[0_24px_80px_-24px_rgba(20,200,210,0.35)]">
            ...
            <p className="mt-4 text-5xl font-semibold text-[var(--fl-text)]">
              {formatUsdFromCents(pricing.standardPriceCents)}
              <span className="text-lg font-bold text-[var(--fl-muted)]">/mo</span>
            </p>
            <p className="mt-2 text-[var(--fl-muted)]">Unlimited inspections, cancel anytime</p>
            <p className="mt-6 text-sm leading-6 text-[var(--fl-muted)]">
        ...
        <section className="mt-16 rounded-2xl border border-[var(--fl-raised)] bg-[var(--fl-surface)] p-8 md:p-10">
          <h2 className="text-2xl font-semibold text-[var(--fl-text)]">Everything included, every plan</h2>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {FEATURES.map((feature) => (
              <div key={feature} className="flex items-start gap-3">
                <span className="mt-1 text-[var(--fl-accent-text)]">✓</span>
                <span className="text-sm leading-6 text-[var(--fl-muted)]">{feature}</span>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="mt-12 text-center">
          <p className="text-[var(--fl-muted)]">
            ...
            <a href="mailto:support@onpointhomeinspect.com" className="font-bold text-[var(--fl-accent-text)] hover:text-[var(--fl-accent-text)]">
              Contact us
            </a>
          </p>
          <p className="mt-2 text-[var(--fl-muted)]">
            Already have an account?{" "}
            <Link href="/login" className="font-bold text-[var(--fl-accent-text)] hover:text-[var(--fl-accent-text)]">
              Log in
            </Link>
          </p>
        </div>
