...
export default async function OwnerPushPage() {
  ...
  const [profiles, companyUsers, nativeTokens, webSubscriptions, events] = await Promise.all([
    safeSelect(admin.from("profiles").select("*").order("created_at", { ascending: false }), "profiles"),
    safeSelect(admin.from("company_users").select("*").order("created_at", { ascending: false }), "company_users"),
    safeSelect(admin.from("app_native_push_tokens").select("*").order("updated_at", { ascending: false }), "app_native_push_tokens"),
    safeSelect(admin.from("app_push_subscriptions").select("*").order("updated_at", { ascending: false }), "app_push_subscriptions"),
    safeSelect(admin.from("app_device_events").select("*").order("created_at", { ascending: false }).limit(200), "app_device_events"),
  ]);

  const userMap = new Map<string, { id: string; label: string; email: string }>();
  ...
  const sentEvents = events.filter((event: any) => String(event?.event_type || "").includes("push"));
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <section className="rounded-3xl border border-yellow-500/40 bg-[#0f172a] p-8 shadow-2xl">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.35em] text-yellow-400">Owner Push Center</p>
              <h1 className="mt-4 text-5xl font-black text-white">Broadcasts & Device Notifications</h1>
              <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-300">
                Send owner-only push announcements to all devices, native iOS devices, web devices, or a single user.
              </p>
            </div>
            <OwnerNav />
          </div>
        ...
        <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          <MetricCard label="Native iOS Tokens" value={String(nativeEnabled.length)} helper="Enabled APNs device tokens." tone="purple" />
          <MetricCard label="Web Push Subscriptions" value={String(webEnabled.length)} helper="Enabled browser push endpoints." tone="teal" />
          <MetricCard label="Known Users" value={String(userMap.size)} helper="Users found from profiles, company users, and tokens." tone="blue" />
          <MetricCard label="Push Events" value={String(sentEvents.length)} helper="Recent push-related analytics events." tone="yellow" />
        </section>

        <Panel title="Send Notification" subtitle="Broadcast update, maintenance, or custom notices.">
          <OwnerPushNotificationCenter
            users={[...userMap.values()].sort((a, b) => a.label.localeCompare(b.label))}
            nativeCount={nativeEnabled.length}
            webCount={webEnabled.length}
          />
        </Panel>

        <Panel title="Recent Push Activity" subtitle="Recent push/token events from app device analytics.">
          {sentEvents.length === 0 ? (
              ...
              {sentEvents.slice(0, 20).map((event: any, index: number) => (
                <div key={event?.id || index} className="rounded-xl border border-slate-700 bg-[#020817]/70 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <p className="font-black text-white">{event?.event_type || "push_event"}</p>
                      <p className="mt-1 text-sm text-slate-400">{event?.path || "/dashboard/owner/push"}</p>
                    </div>
