...
function OwnerNav() {
  return (
    <div className="flex flex-wrap gap-3">
      ...
      <Link href="/dashboard/owner/users" className="rounded-xl border border-cyan-500 px-4 py-3 font-semibold text-[var(--fl-info-text)] transition hover:bg-cyan-500/10">
        👥 Users
      </Link>
      <Link href="/dashboard/owner/devices" className="rounded-xl border border-purple-500 px-4 py-3 font-semibold text-[var(--fl-purple-text)] transition hover:bg-purple-500/10">
        📱 Devices
      </Link>
      <Link href="/dashboard/owner/revenue" className="rounded-xl border border-green-500 px-4 py-3 font-semibold text-[var(--fl-good-text)] transition hover:bg-green-500/10">
        💰 Revenue
      </Link>
      <Link href="/dashboard/owner/push" className="rounded-xl border border-yellow-500 px-4 py-3 font-semibold text-[var(--fl-warn-text)] transition hover:bg-yellow-500/10">
        🔔 Push Center
      </Link>
      <Link href="/dashboard/owner/inspectors" className="rounded-xl border border-orange-500 px-4 py-3 font-semibold text-[var(--fl-warn-text)] transition hover:bg-orange-500/10">
        🧑‍ inspector
      </Link>
      <Link href="/dashboard/owner/live" className="rounded-xl border border-blue-500 px-4 py-3 font-semibold text-[var(--fl-info-text)] transition hover:bg-blue-500/10">
        ⚡ Live
      </Link>
      <Link href="/dashboard/owner/system" className="rounded-xl border border-[var(--fl-faint)] px-4 py-3 font-semibold text-[var(--fl-text)] transition hover:bg-[var(--fl-raised)]">
        🩺 System
      </Link>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-[var(--fl-ground)] px-4 py-8 text-[var(--fl-text)] md:px-6 md:py-10">
      <div className="mx-auto max-w-7xl space-y-8">
        <section className="rounded-2xl border border-blue-500/40 bg-[var(--fl-surface)] p-8 shadow-2xl">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.35em] text-[var(--fl-info-text)]">Owner Live Activity Center</p>
              <h1 className="mt-4 text-5xl font-semibold text-[var(--fl-text)]">Real-Time Business Activity</h1>
                    ...
                    <div key={event?.id || index} className="rounded-xl border border-[var(--fl-line)] bg-[var(--fl-surface-2)] p-4">
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-sm text-[var(--fl-muted)]">{address}</p>
                          {event?.viewer_email && <p className="mt-1 text-xs text-[var(--fl-faint)]">{event.viewer_email}</p>}
                        </div>
                        <div className="text-right">
                          <p className="text-xs font-bold text-[var(--fl-faint)]">{formatDateTime(event?.created_at)}</p>
                          <p className="mt-2 rounded-full border border-blue-500/30 bg-blue-500/10 px-2 py-1 text-xs font-semibold text-[var(--fl-info-text)]">{getViewType(event) || "activity"}</p>
                        </div>
                      </div>
                    </div>
                  );
                ...
                {deviceEvents.slice(0, 40).map((event: any, index: number) => (
                  <div key={event?.id || index} className="rounded-xl border border-[var(--fl-line)] bg-[var(--fl-surface-2)] p-4">
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0">
                        <p className="truncate font-semibold text-[var(--fl-text)]">{event?.event_type || "app_event"}</p>
                        <p className="mt-1 truncate text-sm text-[var(--fl-muted)]">{event?.path || "/"}</p>
                      </div>
                      <p className="shrink-0 text-xs text-[var(--fl-faint)]">{formatDateTime(event?.created_at)}</p>
                    </div>
                  </div>
                ))}
              </div>
