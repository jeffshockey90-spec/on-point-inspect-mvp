  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-[#0f172a] p-6 shadow-2xl">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.24em] text-teal-300">Activation</p>
          <h2 className="mt-1 text-2xl font-black text-white">Inspector Activity</h2>
        ...
        {!loading && (
          <div className="rounded-2xl border border-teal-500/30 bg-teal-500/10 px-4 py-2 text-center">
            <p className="text-3xl font-black leading-none text-teal-200">{total}</p>
            <p className="mt-1 text-[10px] font-black uppercase tracking-wide text-teal-300/80">Active inspectors</p>
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <p className="mt-6 text-sm text-slate-400">Couldn't load inspector activity.</p>
      ) : (
        <>
          {/* Activation funnel */}
