    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="rounded-xl border border-slate-700 bg-[#020617] px-3 py-2 text-sm font-bold text-white outline-none focus:border-teal-400"
        >
      ...
      {visibleRows.length === 0 ? (
        ...
        <div className="space-y-3">
          {visibleRows.map((row) => (
            <div
              key={row.id}
              className="flex flex-col gap-3 rounded-2xl border border-slate-700 bg-[#020617] p-4 sm:flex-row sm:items-center sm:justify-between"
            >
              <div className="min-w-0">
                <Link
                  href={`/reports/${row.id}`}
                  className="break-words font-black text-white hover:text-teal-300"
                >
                  {row.address}
                </Link>
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="rounded-xl border border-teal-500/50 bg-[#0b1220] px-3 py-2 text-sm font-bold text-teal-200 outline-none transition focus:border-teal-400 disabled:opacity-60"
                >
                  {teamMembers.map((member) => (
                    <option key={member.userId} value={member.userId}>
                      {member.label}
                ...
                {savingId === row.id && (
                  <span className="text-xs font-bold text-slate-500">Saving...</span>
                )}
              </div>
            </div>
          ))}
