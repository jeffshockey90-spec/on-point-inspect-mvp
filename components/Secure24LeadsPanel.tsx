    ...
    <section className="rounded-2xl border border-white/10 bg-[#10151e] p-6 shadow-2xl">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-teal-300">Partner</p>
            <h2 className="mt-1 text-2xl font-semibold text-white">Secure 24 Referral Leads</h2>
      ...
      {loading ? (
        ...
        <p className="mt-6 text-sm text-[#8a93a3]">Couldn't load referral leads.</p>
      ) : data.total === 0 && data.errors === 0 ? (
        <p className="mt-6 rounded-2xl border border-white/10 bg-black/30 p-4 text-sm text-[#8a93a3]">
        ...
        <>
          <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-3">
            <div className="rounded-2xl border border-teal-500/30 bg-teal-500/10 p-4">
              <p className="text-3xl font-semibold text-teal-200">{data.total}</p>
              <p className="mt-1 text-[10px] font-semibold uppercase tracking-wide text-teal-300/80">Total leads sent</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
              <p className="text-3xl font-semibold text-white">{data.thisMonth}</p>
              <p className="mt-1 text-[10px] font-semibold uppercase tracking-wide text-[#8a93a3]">This month</p>
            </div>
            <div className={`rounded-2xl border p-4 ${data.errors > 0 ? "border-amber-500/30 bg-amber-500/10" : "border-white/10 bg-black/30"}`}>
              <p className={`text-3xl font-semibold ${data.errors > 0 ? "text-amber-300" : "text-[#59626f]"}`}>{data.errors}</p>
              <p className="mt-1 text-[10px] font-semibold uppercase tracking-wide text-[#8a93a3]">Failed sends</p>
            </div>
          </div>
                ...
                <tbody>
                  {data.byInspector.map((i) => (
                    <tr key={i.inspector_id} className="border-b border-white/5">
                      ...
                      <td className="px-3 py-2 text-right font-semibold text-teal-300">{i.count}</td>
                      <td className="px-3 py-2 text-right text-[#8a93a3]">{shortDate(i.lastAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {data.recent.length > 0 && (
            <div className="mt-6">
              <p className="text-[11px] font-semibold uppercase tracking-wide text-[#59626f]">Recent leads</p>
              <div className="mt-2 space-y-2">
                {data.recent.map((r, idx) => (
                  ...
                  >
                    <span className="font-bold text-[#e8ecf3]">{r.client_name}</span>
                    <span className="text-[#59626f]">{r.inspector_email}</span>
                    <span className="text-[#59626f]">{shortDate(r.at)}</span>
                  </div>
                ))}
              </div>
            </div>
