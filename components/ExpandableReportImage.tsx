...
function ExpandableReportImage({
  ...
  buttonClassName =
    "block w-full overflow-hidden rounded-xl border border-[var(--fl-line)] bg-[var(--fl-surface-2)] text-left focus:outline-none focus:ring-2 focus:ring-cyan-300",
          ...
          >
            ...
            {gallery && (
              <button
                ...
                className="fixed left-2 top-1/2 z-[2147483647] min-h-12 min-w-12 -translate-y-1/2 rounded-full border border-white/30 bg-[var(--fl-surface-2)] px-4 py-3 text-xl font-semibold text-[var(--fl-text)] shadow-xl active:scale-[0.98] active:opacity-80 [touch-action:manipulation] sm:left-4"
                onClick={(event) => {
                  event.stopPropagation();
                  setActiveIndex((current) => (current - 1 + galleryCount) % galleryCount);
                }}
            ...
            {gallery && (
              <button
                ...
                className="fixed right-2 top-1/2 z-[2147483647] min-h-12 min-w-12 -translate-y-1/2 rounded-full border border-white/30 bg-[var(--fl-surface-2)] px-4 py-3 text-xl font-semibold text-[var(--fl-text)] shadow-xl active:scale-[0.98] active:opacity-80 [touch-action:manipulation] sm:right-4"
                onClick={(event) => {
                  event.stopPropagation();
                  setActiveIndex((current) => (current + 1) % galleryCount);
                }}
            ...
            <img
              ...
              alt={displayAlt}
              decoding="async"
              className="max-h-[92vh] max-w-[96vw] rounded-2xl border border-[var(--fl-line)] bg-[var(--fl-surface-2)] object-contain shadow-2xl"
  ...
  return (
    <>
      ...
      >
        ...
        <span className="pointer-events-none absolute bottom-2 right-2 rounded-full border border-white/30 bg-[var(--fl-surface-2)] px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-[var(--fl-text)]">
