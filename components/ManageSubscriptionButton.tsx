...
export default async function BillingPage({
  ...
  const currentStatus = String(profile?.subscription_status || "inactive");
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        <section className="rounded-3xl border border-teal-500/40 bg-[#0f172a] p-8 shadow-2xl">
          ...
          <p className="mt-4 max-w-2xl text-slate-300">
            Your first {freeLimit} real inspections are free. After that, On Point Inspect is {dollars(priceCents)}/month unless the owner has set custom pricing or exempted your account. Subscriptions automatically renew monthly until canceled.
          </p>
        ...
        {params.success && (
          <div className="rounded-2xl border border-green-500/40 bg-green-950/20 p-5 font-bold text-green-200">
            Subscription checkout completed. Your account will update as soon as Stripe confirms the subscription.
          </div>
        )}
        ...
        <section className="grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-2xl border border-slate-700 bg-[#0f172a] p-5">
            ...
            <p className="mt-3 text-3xl font-black text-green-300">
              {exempt ? "Free" : `${dollars(priceCents)}/mo`}
            </p>
          </div>
        </section>
          ...
          <div className="flex flex-wrap items-center justify-between gap-5">
            <div>
              ...
              <p className="mt-3 text-slate-300">
                Current Stripe status: {currentStatus}
              </p>

              {active && (
                <p className="mt-3 max-w-xl text-sm leading-6 text-slate-400">
                  Your subscription renews automatically each month through Stripe. You can update your payment method, view invoices, or cancel any time from the Stripe customer portal.
                </p>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              {exempt ? (
                <Link
                  href="/dashboard"
                  className="rounded-xl bg-teal-500 px-6 py-3 font-black text-slate-950 transition hover:bg-teal-400"
                >
                  Go to Dashboard
                </Link>
              ) : active ? (
                ...
                <>
                  ...
                  <Link
                    href="/support"
                    className="rounded-xl border border-blue-500 px-6 py-3 font-black text-blue-300 transition hover:bg-blue-500/10"
                  >
                    Contact Support
                  </Link>
                </>
