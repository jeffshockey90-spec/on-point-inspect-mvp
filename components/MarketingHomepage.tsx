...
function SectionEyebrow({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-sm font-semibold uppercase tracking-[0.3em] text-teal-400">{children}</p>
  );
}
...
function FieldCaptureMockup() {
  const fieldLabel = "text-[9px] font-semibold uppercase tracking-wide text-[#8a93a3]";
  const fieldBox = "mt-1 rounded-lg border border-white/15 bg-black/40 p-2 text-[10px] leading-snug text-white";

  return (
    <div className="overflow-hidden rounded-2xl border border-[#232b38] bg-black shadow-2xl shadow-black/40">
      ...
      <div className="p-5">
        <div className="relative aspect-[16/10] overflow-hidden rounded-xl border border-white/10 bg-[#131923]">
          <Image
        ...
        <p className="mt-4 text-[9px] font-semibold uppercase tracking-wide text-[#59626f]">
...
function EquipmentAnalyzerMockup() {
  ...
  return (
    <div className="rounded-2xl border border-[#232b38] bg-[#10151e] p-6 shadow-2xl shadow-black/40 md:p-7">
      <p className="text-lg font-bold text-white">AI Equipment Scanner</p>
      <p className="mt-1 text-xs text-[#8a93a3]">Data plate photo scanned automatically</p>
      ...
      <section className="mt-4 rounded-2xl border border-teal-500/40 bg-teal-950/20 p-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          {items.map((item) => (
            <div key={item.label} className="rounded-lg border border-[#232b38] bg-[#0a0e13] p-2.5">
              <p className="text-[9px] font-bold uppercase tracking-wide text-[#8a93a3]">{item.label}</p>
...
function BookingMockup() {
  ...
  return (
    <div className="rounded-2xl border border-[#232b38] bg-[#10151e] p-6 shadow-2xl shadow-black/40 md:p-7">
      ...
      <p className="mt-2 text-sm text-[#8a93a3]">flowinspect.app/book?inspector=your-company</p>

      <div className="mt-5 rounded-2xl border border-[#1a212c] bg-[#0a0e13] p-4">
        <div className="grid grid-cols-7 gap-1 text-center">
          {days.map((day, i) => (
            <span key={i} className="text-[10px] font-semibold uppercase text-[#59626f]">{day}</span>
          ))}
  ...
  return (
    <div className="rounded-2xl border border-[#232b38] bg-[#10151e] p-6 shadow-2xl shadow-black/40 md:p-7">
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-full border border-[#232b38] bg-[#0a0e13] px-3 py-1 text-[10px] font-semibold uppercase text-[#8a93a3]">
          $1,050 Requested
        </span>
      </div>
      ...
      <div className="mt-5 space-y-2.5">
        {items.map((item) => (
          <div key={item.title} className="flex items-center justify-between rounded-xl border border-[#1a212c] bg-[#0a0e13] p-3">
            <div className="min-w-0">
              <p className="truncate text-sm font-bold text-white">{item.title}</p>
              <p className="text-xs text-[#59626f]">Requested: {item.credit}</p>
            </div>
  ...
  return (
    <div className="rounded-2xl border border-[#232b38] bg-[#10151e] p-6 shadow-2xl shadow-black/40 md:p-7">
      ...
      <div className="mt-4 flex items-center justify-between rounded-xl border border-teal-500/30 bg-teal-500/10 p-3">
        ...
        <span className="rounded-lg border border-teal-500/50 bg-[#0a0e13] px-2 py-1 text-xs font-semibold text-teal-300">
          Sarah M. ▾
        </span>
      </div>
    </div>
...
function PhoneFrame({
  ...
  return (
    <div className="w-full max-w-[220px]">
      <div className="relative aspect-[9/19] w-full rounded-[2.25rem] border-[6px] border-[#232b38] bg-black p-1.5 shadow-2xl shadow-black/50">
        <div className="absolute left-1/2 top-1.5 z-10 h-4 w-20 -translate-x-1/2 rounded-full bg-black" />
        <div className="relative h-full w-full overflow-hidden rounded-[1.65rem] bg-[#0a0e13]">
          {children}
        </div>
      </div>
    ...
    <div className="flex flex-wrap items-end justify-center gap-6 sm:gap-8">
      <PhoneFrame label="Field Tool">
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between px-3 pt-6 text-[9px] font-bold text-[#8a93a3]">
            <span>9:41</span>
            <span>●●●</span>
          </div>
          ...
          <div className="space-y-2 p-2.5">
            <div className="rounded-lg border border-teal-500/30 bg-teal-500/10 p-2">
              <p className="text-[7px] font-semibold uppercase tracking-wide text-teal-300">AI Draft Ready</p>
              <p className="mt-0.5 text-[8px] leading-tight text-[#8a93a3]">Missing shingles near chimney flashing...</p>
            </div>
            <div className="flex items-center justify-center rounded-full bg-teal-400 py-2 text-[8px] font-semibold text-slate-950">
              ● Capture Finding
            </div>
          </div>
          <div className="mt-auto flex justify-around border-t border-[#1a212c] bg-[#050816] py-2.5">
            {["🏠", "📷", "📋", "⚙️"].map((icon, i) => (
      ...
      <PhoneFrame label="Push Notifications">
        <div className="flex h-full flex-col">
          ...
          <div className="mt-6 space-y-2 px-2">
            <div className="rounded-xl border border-[#232b38] bg-[#10151e]/95 p-2 shadow-lg">
              ...
              <p className="text-[7px] text-[#8a93a3]">$450 for 711 Sampson Rock Rd</p>
            </div>
            <div className="rounded-xl border border-[#232b38] bg-[#10151e]/95 p-2 shadow-lg">
              ...
              <p className="text-[7px] text-[#8a93a3]">Just now</p>
            </div>
            <div className="rounded-xl border border-[#232b38] bg-[#10151e]/95 p-2 shadow-lg">
              ...
              <p className="text-[7px] text-[#8a93a3]">9:00 AM &middot; 1099 Specks Run Rd</p>
            </div>
          </div>
        </div>
      </PhoneFrame>
...
export default function MarketingHomepage({
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#0a0e13] text-white">
      <header className="border-b border-[#1a212c]/80">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 md:px-8">
          ...
          <nav className="flex items-center gap-2 sm:gap-4">
            {!hideWebPricing && (
              <Link
                href="/pricing"
                className="hidden rounded-xl px-4 py-2 text-sm font-bold text-[#8a93a3] transition hover:text-white sm:block"
              >
                Pricing
              </Link>
            )}
            <Link
              href="/login"
              className="rounded-xl border border-[#232b38] px-4 py-2 text-sm font-semibold text-[#e8ecf3] transition hover:border-[#59626f]"
            >
              Log In
            </Link>
            <Link
            ...
            <div>
              ...
              <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                ...
                {!hideWebPricing && (
                  <Link
                    href="/pricing"
                    className="rounded-2xl border border-[#232b38] bg-[#10151e] px-7 py-4 text-center text-base font-semibold text-[#e8ecf3] transition hover:border-teal-400 active:scale-[0.98]"
                  >
                    See Pricing
                  </Link>
                )}
              ...
              <p className="mt-5 text-sm text-[#59626f]">
                Your first 3 real inspections are free. No credit card to sign up.
              </p>
            </div>

            <div className="relative">
              <div className="rounded-2xl border border-[#232b38] bg-[#10151e] p-6 shadow-2xl shadow-black/40 md:p-8">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <div className="rounded-2xl border border-teal-500/30 bg-teal-500/10 p-4">
                    ...
                    <p className="mt-1 text-3xl font-semibold text-white">3</p>
                  </div>
                  <div className="rounded-2xl border border-[#232b38] bg-[#0a0e13] p-4">
                    <p className="text-xs font-bold uppercase text-[#8a93a3]">Unpaid</p>
                    <p className="mt-1 text-3xl font-semibold text-white">$820</p>
                  </div>
                </div>

                <div className="mt-4 space-y-3">
                  <div className="flex items-center justify-between rounded-2xl border border-[#1a212c] bg-[#0a0e13] p-4">
                    <div>
                      <p className="font-bold text-white">711 Sampson Rock Rd</p>
                      <p className="text-xs text-[#59626f]">Report published &middot; Payment received</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between rounded-2xl border border-[#1a212c] bg-[#0a0e13] p-4">
                    <div>
                      ...
                      <p className="text-xs text-[#59626f]">Agreement sent &middot; Awaiting signature</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between rounded-2xl border border-[#1a212c] bg-[#0a0e13] p-4">
                    <div>
                      ...
                      <p className="text-xs text-[#59626f]">Seller responded &middot; 4 of 5 items agreed</p>
                    </div>
      ...
      {SPOTLIGHTS.map((spotlight, index) => (
        <section
          key={spotlight.title}
          className={`border-t border-[#1a212c]/80 ${index % 2 === 1 ? "bg-[#03060f]" : ""}`}
        >
          <div className="mx-auto max-w-7xl px-5 py-20 md:px-8">
            ...
            >
              <div>
                ...
                <p className="mt-5 text-base leading-8 text-[#8a93a3]">{spotlight.body}</p>

                <ul className="mt-6 space-y-3">
                  {spotlight.bullets.map((bullet) => (
                    <li key={bullet} className="flex items-start gap-3 text-sm text-[#8a93a3]">
      ...
      <section className="border-t border-[#1a212c]/80 bg-[#03060f]">
        <div className="mx-auto max-w-7xl px-5 py-20 md:px-8">
          ...
          <div className="mt-12 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
            {CORE_FEATURES.map((feature) => (
              <div
                key={feature.title}
                className="rounded-2xl border border-[#1a212c] bg-[#10151e] p-6 transition hover:-translate-y-0.5 hover:border-teal-500/60"
              >
                <div className="text-3xl">{feature.icon}</div>
                <h3 className="mt-4 text-lg font-semibold text-white">{feature.title}</h3>
                <p className="mt-2 text-sm leading-6 text-[#8a93a3]">{feature.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-[#1a212c]/80">
        <div className="mx-auto max-w-7xl px-5 py-20 md:px-8">
          <div className="max-w-2xl">
          ...
          <div className="mt-12 grid gap-10 md:grid-cols-2 xl:grid-cols-4">
            {MORE_FEATURES.map((group) => (
              <div key={group.category}>
                <h3 className="text-xs font-semibold uppercase tracking-[0.2em] text-teal-400">
                  {group.category}
                </h3>
                <div className="mt-4 space-y-5">
                  {group.items.map((item) => (
                    <div key={item.title}>
                      ...
                      <p className="mt-1 text-sm leading-6 text-[#8a93a3]">{item.body}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="mt-12 grid gap-8 md:grid-cols-3">
            {STEPS.map((step) => (
              <div key={step.number}>
                ...
                <p className="mt-3 text-sm leading-7 text-[#8a93a3]">{step.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-[#1a212c]/80">
        <div className="mx-auto max-w-4xl px-5 py-20 text-center md:px-8">
          ...
          <p className="mx-auto mt-5 max-w-xl text-lg leading-8 text-[#8a93a3]">
          ...
          <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            ...
            {!hideWebPricing && (
              <Link
                href="/pricing"
                className="rounded-2xl border border-[#232b38] px-8 py-4 text-base font-semibold text-[#e8ecf3] transition hover:border-teal-400 active:scale-[0.98]"
              >
                View Pricing
              </Link>
            )}
      ...
      <footer className="border-t border-[#1a212c]/80">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-5 py-10 text-sm text-[#59626f] md:flex-row md:items-center md:justify-between md:px-8">
          <div className="flex items-center gap-2">
            <img src="/flow-logo-mark.png" alt="FLOW" className="h-6 w-auto rounded" />
            <span className="font-semibold text-[#8a93a3]">FLOW</span>
          </div>

          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            {!hideWebPricing && (
              <Link href="/pricing" className="hover:text-[#8a93a3]">Pricing</Link>
            )}
            <Link href="/login" className="hover:text-[#8a93a3]">Log In</Link>
            <Link href="/signup" className="hover:text-[#8a93a3]">Sign Up</Link>
            <a href="mailto:support@onpointhomeinspect.com" className="hover:text-[#8a93a3]">Support</a>
            <Link href="/terms" className="hover:text-[#8a93a3]">Terms</Link>
            <Link href="/privacy" className="hover:text-[#8a93a3]">Privacy</Link>
          </div>
