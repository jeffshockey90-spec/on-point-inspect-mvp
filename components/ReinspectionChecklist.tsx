...
const STATUSES: { key: string; label: string; on: string }[] = [
  ...
  { key: "not_evaluated", label: "Not evaluated", on: "border-slate-400 bg-slate-500/20 text-slate-200" },
];
  ...
  return (
    <section className="rounded-2xl border border-amber-500/40 bg-amber-950/15 p-5 shadow-xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-xs font-black uppercase tracking-[0.22em] text-amber-300">Re-inspection</p>
          <h2 className="mt-1 text-2xl font-black text-white">Verify Prior Findings</h2>
      ...
      {loading ? (
        ...
        <p className="mt-5 text-sm text-slate-400">No findings carried over.</p>
      ) : (
        <div className="mt-5 space-y-2">
          {findings.map((f) => (
            <div
              key={f.id}
              className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-700 bg-black/30 p-3"
            >
                ...
                {STATUSES.map((s) => {
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-lg border px-2.5 py-1.5 text-[11px] font-black transition disabled:opacity-50 ${
                        active ? s.on : "border-slate-700 text-slate-400 hover:text-slate-200"
                      }`}
                    >
                      {s.label}
                    </button>
