    ...
    <section className="rounded-2xl border border-[var(--fl-raised)] bg-gradient-to-br from-[var(--fl-surface)] to-[var(--fl-surface)] p-5 shadow-xl sm:p-6 md:p-8">
      ...
      {!data.available ? (
        ...
        <>
          ...
          <div className="mt-4 overflow-hidden rounded-xl border border-[var(--fl-line)]">
            <div className="max-h-72 overflow-y-auto">
              <table className="w-full text-left text-sm">
                <thead className="sticky top-0 bg-[var(--fl-ground)] text-[11px] uppercase tracking-wide text-[var(--fl-faint)]">
                  <tr>
                ...
                <tbody className="divide-y divide-[var(--fl-raised)]">
                  {data.events.map((e) => (
                    <tr key={e.id} className="text-[var(--fl-muted)]">
                      <td className="whitespace-nowrap px-3 py-2 text-[var(--fl-muted)]">
                        {timeAgo(e.created_at)}
                      </td>
                      <td className="px-3 py-2 font-semibold text-[var(--fl-text)]">{label(e.event_type)}</td>
                      <td className="whitespace-nowrap px-3 py-2 font-mono text-xs text-[var(--fl-muted)]">
                        {e.ip || "unknown"}
                      </td>
                    </tr>
                  ))}
