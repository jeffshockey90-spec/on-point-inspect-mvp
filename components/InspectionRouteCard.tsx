...
export default function InspectionRouteCard({
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-700 bg-[#0f172a] shadow-xl">
      {embedSrc && (
        <iframe
          src={embedSrc}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 break-words text-sm text-slate-300">{fullAddress}</p>

          {hasDistance ? (
            <p className="mt-1 text-sm font-black text-teal-300">
              {distanceMiles ? `${distanceMiles} mi` : ""}
              {distanceMiles && driveMinutes ? " · " : ""}
              {driveMinutes ? `~${driveMinutes} min drive` : ""}
            </p>
          ) : officeAddress ? (
            <p className="mt-1 text-xs text-slate-500">Calculating drive distance...</p>
          ) : (
        ...
        <a
          ...
          className="shrink-0 rounded-xl bg-teal-500 px-5 py-3 text-sm font-black text-slate-950 transition hover:bg-teal-400 active:scale-[0.98]"
        >
          🚗 Start Drive
        </a>
      </div>
