  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="rounded-xl border border-amber-400/50 px-4 py-2 text-xs font-semibold text-amber-200">
          {open ? "Hide" : "Show"}
        </span>
      </div>
...
function ReviewCard({
  ...
  return (
    <div className="w-full max-w-full overflow-hidden rounded-2xl border border-[#232b38] bg-[#10151e] p-3 sm:p-4">
      <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
        <h3 className="min-w-0 break-words text-base font-semibold text-cyan-200">
          {finding.title || "Offline Field Note"}
        </h3>
        <span className="shrink-0 rounded-full border border-amber-400/50 bg-amber-500/10 px-3 py-1 text-[11px] font-semibold text-amber-200">
          Needs review
        </span>
      </div>
          ...
          {photos.slice(0, 6).map((photo: any, index: number) => {
            ...
            if (photoIsVideo(photo)) {
              return (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="relative flex h-20 w-20 items-center justify-center rounded-lg border border-[#232b38] bg-black text-xs font-semibold text-white"
                >
                  ▶ Video
                </a>
              );
            ...
            return (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="block h-20 w-20 overflow-hidden rounded-lg border border-[#232b38] bg-black"
              >
      ...
      <div className="mb-3 grid gap-3 sm:grid-cols-2">
        <label className="block">
          <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-[#8a93a3]">
            Section
          </p>
          <select
            value={section}
            ...
            className="min-h-[44px] w-full rounded-xl border border-[#232b38] bg-[#0a0e13] px-3 py-2 text-sm font-bold text-white outline-none focus:border-cyan-400 disabled:opacity-60"
          >
            {options.map((item) => (
      ...
      <div className="mb-3 space-y-2 rounded-xl border border-[#1a212c] bg-[#10151e] p-3">
      ...
      {noteOpen && (
        <div className="mb-3 rounded-xl border border-purple-500/50 bg-purple-950/20 p-3">
          ...
          <textarea
            ...
            className="min-h-[96px] w-full rounded-xl border border-purple-500/40 bg-[#0a0e13] px-3 py-2 text-sm font-bold text-white outline-none focus:border-purple-300 disabled:opacity-60"
          />
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {!noteOpen ? (
          <button
            ...
            disabled={approving || repolishing}
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-purple-500 bg-purple-500/10 px-5 py-3 text-sm font-semibold text-purple-200 transition active:scale-[0.98] hover:bg-purple-500/20 disabled:cursor-not-allowed disabled:opacity-60 [touch-action:manipulation]"
          >
            Edit note & re-polish
          </button>
        ) : (
          <>
            ...
            <button
              ...
              disabled={repolishing}
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#232b38] px-5 py-3 text-sm font-semibold text-[#e8ecf3] transition active:scale-[0.98] hover:bg-[#1a212c] disabled:cursor-not-allowed disabled:opacity-60 [touch-action:manipulation]"
            >
              Cancel
            </button>
          </>
...
function WriteUpRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div>
      ...
      <p className="mt-0.5 whitespace-pre-wrap text-sm leading-6 text-[#e8ecf3]">
        {value}
      </p>
    </div>
  );
