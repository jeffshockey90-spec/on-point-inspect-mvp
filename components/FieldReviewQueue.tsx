      ...
      >
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <h2 className="text-base font-black text-amber-100 sm:text-lg">
              Field Review · {count} finding{count === 1 ? "" : "s"} to approve
            </h2>
            <p className="mt-0.5 text-xs font-bold text-amber-200/70">
        ...
        <span className="rounded-xl border border-amber-400/50 px-4 py-2 text-xs font-black text-amber-200">
          {open ? "Hide" : "Show"}
        </span>
      </div>
        ...
        <div className="space-y-4 border-t border-amber-500/30 p-3 sm:p-4">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-amber-400 px-5 py-3 text-sm font-black text-slate-950 transition active:scale-[0.98] hover:bg-amber-300 disabled:cursor-not-allowed disabled:opacity-60 [touch-action:manipulation]"
            >
              {approvingAll && (
...
function ReviewCard({
  ...
  return (
    <div className="w-full max-w-full overflow-hidden rounded-2xl border border-slate-700 bg-[#0f172a] p-3 sm:p-4">
      <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
        <h3 className="min-w-0 break-words text-base font-black text-cyan-200">
          {finding.title || "Offline Field Note"}
        </h3>
        <span className="shrink-0 rounded-full border border-amber-400/50 bg-amber-500/10 px-3 py-1 text-[11px] font-black text-amber-200">
          Needs review
        </span>
      </div>
          ...
          {photos.slice(0, 6).map((photo: any, index: number) => {
            ...
            if (photoIsVideo(photo)) {
              return (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="relative flex h-20 w-20 items-center justify-center rounded-lg border border-slate-700 bg-black text-xs font-black text-white"
                >
                  ▶ Video
                </a>
              );
            ...
            return (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="block h-20 w-20 overflow-hidden rounded-lg border border-slate-700 bg-black"
              >
      ...
      <div className="mb-3 grid gap-3 sm:grid-cols-2">
        <label className="block">
          ...
          <select
            ...
            className="min-h-[44px] w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm font-bold text-white outline-none focus:border-cyan-400 disabled:opacity-60"
          >
            {options.map((item) => (
      ...
      <div className="mb-3 space-y-2 rounded-xl border border-slate-800 bg-[#0b1220] p-3">
      ...
      {noteOpen && (
        <div className="mb-3 rounded-xl border border-purple-500/50 bg-purple-950/20 p-3">
          <p className="mb-2 text-xs font-black uppercase tracking-wide text-purple-300">
          ...
          <textarea
            ...
            className="min-h-[96px] w-full rounded-xl border border-purple-500/40 bg-slate-950 px-3 py-2 text-sm font-bold text-white outline-none focus:border-purple-300 disabled:opacity-60"
          />
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          onClick={approve}
          disabled={approving || repolishing}
          className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-500 px-5 py-3 text-sm font-black text-slate-950 transition active:scale-[0.98] hover:bg-teal-400 disabled:cursor-not-allowed disabled:opacity-60 [touch-action:manipulation]"
        >
          {approving && (
            <span className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" />
          )}
        ...
        {!noteOpen ? (
          <button
            ...
            disabled={approving || repolishing}
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-purple-500 bg-purple-500/10 px-5 py-3 text-sm font-black text-purple-200 transition active:scale-[0.98] hover:bg-purple-500/20 disabled:cursor-not-allowed disabled:opacity-60 [touch-action:manipulation]"
          >
            Edit note & re-polish
          </button>
        ) : (
          <>
            <button
              ...
              onClick={repolish}
              disabled={repolishing}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-purple-500 px-5 py-3 text-sm font-black text-white transition active:scale-[0.98] hover:bg-purple-400 disabled:cursor-not-allowed disabled:opacity-60 [touch-action:manipulation]"
            >
              {repolishing && (
                <span className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" />
              )}
            ...
            <button
              ...
              disabled={repolishing}
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-600 px-5 py-3 text-sm font-black text-slate-200 transition active:scale-[0.98] hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60 [touch-action:manipulation]"
            >
              Cancel
            </button>
          </>
...
function WriteUpRow({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div>
      ...
      <p className="mt-0.5 whitespace-pre-wrap text-sm leading-6 text-slate-200">
        {value}
      </p>
    </div>
  );
