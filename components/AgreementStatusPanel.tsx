...
function Badge({ children, tone }: { children: React.ReactNode; tone: "green" | "yellow" | "teal" | "slate" }) {
  const classes =
    tone === "green"
...
export default function AgreementStatusPanel({ inspectionId }: { inspectionId: string }) {
  ...
  const requiredContacts = useMemo(() => contacts.filter((contact) => Boolean(contact.agreement_required)), [contacts]);
  const unsignedRequiredContacts = useMemo(() => requiredContacts.filter((contact) => !contact.agreement_signed), [requiredContacts]);
  const allRequiredSigned = requiredContacts.length > 0 && unsignedRequiredContacts.length === 0;
  ...
  return (
    <section className="mb-6 w-full max-w-full overflow-hidden rounded-3xl border border-slate-700 bg-[#071224] shadow-2xl shadow-black/20">
      <div className="border-b border-slate-800 bg-gradient-to-r from-[#0f172a] via-[#0b1628] to-[#071224] p-4 sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <p className="text-xs font-black uppercase tracking-[0.28em] text-teal-400">Agreement Status</p>
            <h2 className="mt-2 text-2xl font-black text-white">Signature Tracking</h2>
      ...
      <div className="p-4 sm:p-5">
        {loading && <p className="rounded-2xl border border-slate-700 bg-[#020817]/80 p-4 text-sm text-slate-400">Loading agreement status...</p>}
        ...
        {!loading && requiredContacts.length > 0 && (
          <div className="space-y-3">
            {requiredContacts.map((contact) => {
              ...
              return (
                <article key={contact.id} className="rounded-2xl border border-slate-700 bg-[#020817]/80 p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="break-words text-base font-black text-white">{contact.name}</p>
                        <Badge tone="teal">{contact.role || "client"}</Badge>
                        <Badge tone={signed ? "green" : "yellow"}>{signed ? "Signed" : "Unsigned"}</Badge>
                        <Badge tone="slate">Required</Badge>
                      </div>
                      <p className="mt-2 break-all text-sm text-slate-400">{contact.email}</p>
                    </div>
                    ...
                    {!signed && (
                      <button
                        ...
                        disabled={sendingReminder !== null}
                        className="w-full rounded-xl border border-yellow-500/70 px-4 py-2 text-sm font-black text-yellow-300 transition hover:bg-yellow-500/10 disabled:opacity-50 sm:w-auto"
                      >
