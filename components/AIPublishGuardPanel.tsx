      ...
      <button type="button" onClick={() => setOpen((value) => !value)} className="flex w-full items-start justify-between gap-3 p-3 text-left">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-black">{resolved ? "✓ " : ""}{issue.title}</span>
            <span className="rounded-full border border-current/30 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide">{priorityLabel(issue)}</span>
            {issue.section && <span className="rounded-full border border-current/30 px-2 py-0.5 text-[10px] font-black">{issue.section}</span>}
          </div>
        ...
        <div className="border-t border-current/20 p-3">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className={`rounded-lg border p-3 ${confidenceTone(confidence)}`}>
              <p className="text-[10px] font-black uppercase tracking-wide opacity-80">AI Confidence</p>
              <p className="mt-1 text-xl font-black">{confidence}%</p>
            </div>
            <div className="rounded-lg border border-current/20 bg-black/20 p-3">
              <p className="text-[10px] font-black uppercase tracking-wide opacity-80">Liability Risk</p>
              <p className="mt-1 text-base font-black uppercase">{issue.liabilityRisk || (issue.severity === "critical" ? "high" : issue.severity === "warning" ? "medium" : "low")}</p>
            </div>
            <div className="rounded-lg border border-current/20 bg-black/20 p-3">
              <p className="text-[10px] font-black uppercase tracking-wide opacity-80">Status</p>
              <p className="mt-1 text-base font-black">{resolved ? "Reviewed" : issue.blocking ? "Blocking" : "Needs Review"}</p>
            </div>
          </div>

          <div className="mt-3 rounded-lg border border-current/20 bg-black/20 p-3">
            <p className="text-xs font-black uppercase tracking-wide opacity-80">Expected evidence</p>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
            <button type="button" onClick={openSection} className="rounded-lg border border-white/15 bg-white/10 px-3 py-2 text-xs font-black hover:bg-white/15">Open Section</button>
            <button type="button" onClick={openCamera} className="rounded-lg border border-cyan-400/30 bg-cyan-400/10 px-3 py-2 text-xs font-black text-cyan-100 hover:bg-cyan-400/20">Open AI Camera</button>
            <button type="button" onClick={startFinding} className="rounded-lg border border-rose-400/30 bg-rose-400/10 px-3 py-2 text-xs font-black text-rose-100 hover:bg-rose-400/20">Add Finding</button>
            <button type="button" onClick={addLimitation} className="rounded-lg border border-amber-400/30 bg-amber-400/10 px-3 py-2 text-xs font-black text-amber-100 hover:bg-amber-400/20">Add Limitation</button>
            <button type="button" onClick={markReviewed} disabled={resolved} className="rounded-lg border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-xs font-black text-emerald-100 hover:bg-emerald-400/20 disabled:opacity-50">{resolved ? "Reviewed" : "Mark Reviewed"}</button>
            <button type="button" onClick={() => setOpen(false)} className="rounded-lg border border-slate-500/30 bg-slate-500/10 px-3 py-2 text-xs font-black text-slate-200 hover:bg-slate-500/20">Collapse</button>
          </div>
        </div>
      )}
    </article>
  ...
  return (
    <section className="rounded-2xl border border-rose-500/40 bg-rose-950/20 p-4 shadow-xl">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.22em] text-rose-300">AI Publish Guard</p>
          <h2 className="mt-1 text-2xl font-black text-white">AI Quality Control Center</h2>
          <p className="mt-2 text-sm leading-6 text-slate-300">Finds missing safety evidence, incomplete systems, contradictions, missing media, and liability-sensitive documentation—then gives you one-tap ways to resolve each item.</p>
        </div>
        <button type="button" onClick={runGuard} disabled={loading || !inspectionId} className="rounded-xl bg-rose-500 px-4 py-3 text-sm font-black text-white transition hover:bg-rose-400 disabled:cursor-not-allowed disabled:opacity-60">{loading ? "Checking..." : result ? "Run Again" : "Run Publish Guard"}</button>
      </div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3">
        <div className={`rounded-xl border p-4 ${effectiveScore === null ? "border-slate-700 bg-[#020817]/70 text-slate-300" : scoreTone(effectiveScore)}`}>
          <p className="text-xs font-black uppercase tracking-wide opacity-80">Inspection Completeness</p>
          <p className="mt-1 text-3xl font-black">{effectiveScore === null ? "—" : effectiveScore}{effectiveScore !== null && <span className="text-base opacity-80"> / 100</span>}</p>
        ...
        <div className={`rounded-xl border p-4 ${recommendationTone(result?.recommendation)}`}>
          <p className="text-xs font-black uppercase tracking-wide opacity-80">Recommendation</p>
          <p className="mt-1 text-xl font-black">{activeIssues.length === 0 && result ? "Ready" : result?.recommendation || "Not checked yet"}</p>
        ...
        <div className="rounded-xl border border-slate-700 bg-[#020817]/70 p-4">
          <p className="text-xs font-black uppercase tracking-wide text-slate-400">Reviewed</p>
          <p className="mt-2 text-sm text-slate-300">Findings: <span className="font-black text-white">{result?.findingCount ?? "—"}</span></p>
          <p className="mt-1 text-sm text-slate-300">Equipment: <span className="font-black text-white">{result?.equipmentCount ?? "—"}</span></p>
          <p className="mt-1 text-sm text-slate-300">Media: <span className="font-black text-white">{result?.photoCount ?? "—"}</span></p>
        </div>
      </div>

      {result && (
        <div className="mt-5 grid gap-4 xl:grid-cols-[0.8fr_1.4fr]">
          <aside className="rounded-xl border border-slate-700 bg-[#020817]/70 p-4">
            <h3 className="text-sm font-black uppercase tracking-wide text-slate-400">System Status</h3>
            <div className="mt-3 space-y-2">
              {groupedSections.length === 0 ? <p className="text-sm font-bold text-emerald-300">✓ All reviewed systems are clear.</p> : groupedSections.map(([section, meta]) => (
                <button key={section} type="button" onClick={() => findAndScrollToSection(section)} className="flex w-full items-center justify-between rounded-lg border border-slate-700 bg-black/20 px-3 py-2 text-left hover:bg-white/5">
                  <span className="text-sm font-black text-white">{section}</span>
                  <span className={meta.critical ? "text-xs font-black text-red-300" : "text-xs font-black text-amber-300"}>{meta.critical ? "●" : "⚠"} {meta.issueCount}</span>
                </button>
              ))}
            </div>
            <h3 className="mt-5 text-sm font-black uppercase tracking-wide text-slate-400">Final Suggestions</h3>
            <ul className="mt-3 space-y-2 text-sm leading-6 text-slate-300">{(result.suggestions?.length ? result.suggestions : ["Perform final inspector review before publishing."]).map((item, index) => <li key={index}>✓ {item}</li>)}</ul>
          </aside>

          <div className="rounded-xl border border-slate-700 bg-[#020817]/70 p-4">
            <div className="flex items-center justify-between gap-3">
              <h3 className="text-sm font-black uppercase tracking-wide text-slate-400">Actionable Publish Guard Issues</h3>
              <span className="rounded-full border border-slate-600 bg-black/30 px-2 py-1 text-xs font-black text-slate-300">{activeIssues.length}</span>
            </div>
            ...
            {activeIssues.length > displayedIssues.length && <p className="mt-3 text-xs font-bold text-slate-400">Showing the first {displayedIssues.length} of {activeIssues.length} unresolved issues.</p>}
          </div>
        </div>
      )}
    </section>
