...
export default function GlobalSearchButton() {
  return (
    <button
      ...
      className="fixed bottom-28 right-4 z-40 flex items-center gap-2 rounded-full border border-[var(--fl-line)] bg-[var(--fl-ground)] px-4 py-3 text-sm font-semibold text-[var(--fl-muted)] shadow-xl shadow-black/40 backdrop-blur transition active:scale-95 hover:border-teal-400/60 hover:text-[var(--fl-accent-text)] md:bottom-6 md:right-6 [touch-action:manipulation]"
    >
      <Search className="h-4 w-4" strokeWidth={2.5} />
      <span className="hidden sm:inline">Search</span>
      <kbd className="hidden rounded-md border border-[var(--fl-line)] px-1.5 py-0.5 text-[11px] font-semibold text-[var(--fl-faint)] md:inline">
        ⌘K
      </kbd>
    </button>
  );
