    ...
    <section className="space-y-3 rounded-2xl border border-violet-400/40 bg-violet-500/10 p-3 text-white">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-violet-300">AI Vision Platform</p>
          <h3 className="mt-1 truncate text-lg font-black">{section}</h3>
          <p className="mt-1 text-xs leading-5 text-slate-300">Tracks what AI has seen, what has evidence, and where to inspect next.</p>
        </div>
        <div className={`shrink-0 rounded-full border px-3 py-2 text-sm font-black ${scoreTone(current?.score || 0)}`}>
          {current?.score || 0}%
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="rounded-xl border border-emerald-400/30 bg-black/20 p-2">
          <p className="text-lg font-black text-emerald-200">{current?.covered.length || 0}</p>
          <p className="text-[10px] font-bold text-slate-300">Covered</p>
        </div>
        <div className="rounded-xl border border-yellow-400/30 bg-black/20 p-2">
          <p className="text-lg font-black text-yellow-200">{current?.missing.length || 0}</p>
          <p className="text-[10px] font-bold text-slate-300">Missing</p>
        </div>
        <div className="rounded-xl border border-cyan-400/30 bg-black/20 p-2">
          <p className="text-lg font-black text-cyan-200">{sectionObjects.length}</p>
          <p className="text-[10px] font-bold text-slate-300">Objects</p>
        </div>
      </div>

      {current?.missing.length ? (
        <div className="rounded-xl border border-yellow-400/30 bg-yellow-500/10 p-3">
          <p className="text-xs font-black uppercase tracking-[0.16em] text-yellow-200">Still Missing</p>
      ...
      {sectionObjects.length ? (
        <details open className="rounded-xl border border-slate-700 bg-black/20 p-3">
          <summary className="cursor-pointer text-sm font-black text-slate-100">Digital Twin objects ({sectionObjects.length})</summary>
          <div className="mt-3 space-y-2">
            {sectionObjects.map((item) => (
              <div key={item.key} className="flex items-center justify-between gap-3 rounded-lg border border-white/10 bg-black/20 px-3 py-2">
                <div className="min-w-0">
                  <p className="truncate text-sm font-black">{item.documented ? "✓" : "◉"} {item.label}</p>
                  <p className="text-[10px] text-slate-400">Seen {item.seenCount}× · {Math.round(item.confidence * 100)}% confidence</p>
                </div>
                <span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-black ${item.documented ? "bg-emerald-400 text-black" : "bg-cyan-400/15 text-cyan-200"}`}>
      ...
      {snapshot.route.length ? (
        <div className="rounded-xl border border-cyan-400/30 bg-cyan-500/10 p-3">
          <p className="text-xs font-black uppercase tracking-[0.16em] text-cyan-200">Next Best Inspection Path</p>
          <div className="mt-2 space-y-2">
            {snapshot.route.slice(0, 3).map((step, index) => (
              <div key={step.section} className="flex items-start gap-3">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-cyan-400 text-xs font-black text-black">{index + 1}</span>
                <div>
                  <p className="text-sm font-black text-white">{step.section}</p>
      ...
      {habitPrompts.length ? (
        <div className="rounded-xl border border-purple-400/30 bg-purple-500/10 p-3">
          <p className="text-xs font-black uppercase tracking-[0.16em] text-purple-200">Inspector Memory</p>
          <p className="mt-1 text-xs text-slate-300">You commonly document these items, but they have not been seen here yet.</p>
      ...
      <button type="button" onClick={() => void refresh()} disabled={loading} className="min-h-11 w-full rounded-xl border border-violet-300/40 bg-black/20 px-3 py-2 text-sm font-black text-violet-100 disabled:opacity-50">
