...
export default function ScheduleReminderSettings() {
  ...
  return (
    <section className="mb-6 overflow-hidden rounded-2xl border border-teal-500/30 bg-[var(--fl-surface)] shadow-2xl shadow-black/20">
      <div className="flex flex-col gap-4 border-b border-[var(--fl-raised)] px-5 py-5 sm:flex-row sm:items-start sm:justify-between sm:px-6">
      ...
      <div className="space-y-6 px-5 py-5 sm:px-6">
        {GROUPS.map((group) => (
          <section key={group.title}>
            <h3 className="mb-2 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--fl-faint)]">
              {group.title}
            </h3>

            <div className="overflow-hidden rounded-2xl border border-[var(--fl-raised)] bg-[var(--fl-surface-2)]">
              {group.options.map((option, index) => {
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-center justify-between gap-4 px-4 py-3 text-left transition hover:bg-[var(--fl-surface-2)] disabled:cursor-not-allowed ${
                      index > 0 ? "border-t border-[var(--fl-raised)]" : ""
                    }`}
                  >
                    <span className="min-w-0">
                      <span className="block text-sm font-semibold text-[var(--fl-text)] sm:text-base">
                        {option.title}
                      </span>
                      <span className="mt-0.5 block text-xs leading-5 text-[var(--fl-muted)] sm:text-sm">
                        {option.description}
                      </span>
                      {isSaving ? (
                        <span className="mt-1 block text-xs font-bold text-[var(--fl-accent-text)]">
                          Saving…
                        </span>
                      ) : null}
                    </span>
          ...
          <div className="overflow-hidden rounded-2xl border border-[var(--fl-raised)] bg-[var(--fl-surface-2)]">
            <div className="flex w-full items-center justify-between gap-4 px-4 py-3">
              ...
              <select
                ...
                className="shrink-0 rounded-xl border border-[var(--fl-line)] bg-[var(--fl-ground)] px-3 py-2 text-sm font-bold text-[var(--fl-text)] disabled:cursor-not-allowed disabled:opacity-50"
              >
        ...
        <p className="text-xs leading-5 text-[var(--fl-faint)]">
          New accounts and previously unset preferences default to On.
        </p>

        {message ? (
          <p className="rounded-xl border border-[var(--fl-line)] bg-[var(--fl-surface-2)] px-3 py-2 text-sm font-bold text-[var(--fl-text)]">
            {message}
          </p>
        ) : null}
      </div>
