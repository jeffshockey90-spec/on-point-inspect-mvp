...
export default function ReportTemplateSwitcher({
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-3 rounded-xl border border-[var(--fl-raised)] bg-[var(--fl-surface)] px-3 py-2.5">
      <span className="fl-lbl">Template</span>
      <select
        value={value}
        disabled={busy}
        onChange={(e) => apply(e.target.value)}
        className="rounded-lg border border-[var(--fl-line)] bg-[var(--fl-ground)] px-3 py-1.5 text-sm font-bold text-[var(--fl-text)] outline-none focus:border-teal-400 disabled:opacity-60"
      >
      ...
      {busy && <span className="text-xs text-[var(--fl-muted)]">Applying…</span>}
      <span className="ml-auto text-xs text-[var(--fl-faint)]">Switching never deletes findings.</span>
    </div>
  );
}
