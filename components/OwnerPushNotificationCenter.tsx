  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-[#232b38] bg-[#131923] p-5">
        <p className="text-sm font-semibold uppercase tracking-wide text-teal-300">Broadcast Presets</p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="block">
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#8a93a3]">Send To</p>
          <select value={target} onChange={(event) => { setTarget(event.target.value as TargetMode); setSelectedUser(""); }} className="w-full rounded-xl border border-[#232b38] bg-[#0a0e13] p-3 text-white outline-none focus:border-teal-400">
        ...
        {target === "user" ? (
          <label className="block">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#8a93a3]">User</p>
            <select value={selectedUser} onChange={(event) => setSelectedUser(event.target.value)} className="w-full rounded-xl border border-[#232b38] bg-[#0a0e13] p-3 text-white outline-none focus:border-teal-400">
      ...
      <label className="block"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#8a93a3]">Message</p><textarea value={body} onChange={(event) => setBody(event.target.value)} rows={5} className="w-full rounded-xl border border-[#232b38] bg-[#0a0e13] p-3 text-white outline-none focus:border-teal-400" /></label>
      ...
      {result && <pre className="max-h-72 overflow-auto rounded-xl border border-[#232b38] bg-[#0a0e13] p-4 text-xs text-[#8a93a3]">{JSON.stringify(result, null, 2)}</pre>}
    </div>
  );
}

function Stat({ label, value, className }: { label: string; value: number; className: string }) {
  return <div className={`rounded-xl border p-4 ${className}`}><p className="text-xs font-semibold uppercase text-[#8a93a3]">{label}</p><p className="mt-2 text-3xl font-semibold text-white">{value}</p></div>;
}
function TextInput({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label className="block"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[#8a93a3]">{label}</p><input value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-xl border border-[#232b38] bg-[#0a0e13] p-3 text-white outline-none focus:border-teal-400" /></label>;
}
