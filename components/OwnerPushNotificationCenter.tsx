  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border border-[var(--fl-line)] bg-[var(--fl-surface-2)] p-5">
        <p className="text-sm font-semibold uppercase tracking-wide text-[var(--fl-accent-text)]">Broadcast Presets</p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <label className="block">
          ...
          <select value={target} onChange={(event) => { setTarget(event.target.value as TargetMode); setSelectedUser(""); }} className="w-full rounded-xl border border-[var(--fl-line)] bg-[var(--fl-ground)] p-3 text-[var(--fl-text)] outline-none focus:border-teal-400">
        ...
        {target === "user" ? (
          <label className="block">
            <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[var(--fl-muted)]">User</p>
            <select value={selectedUser} onChange={(event) => setSelectedUser(event.target.value)} className="w-full rounded-xl border border-[var(--fl-line)] bg-[var(--fl-ground)] p-3 text-[var(--fl-text)] outline-none focus:border-teal-400">
      ...
      {result && <pre className="max-h-72 overflow-auto rounded-xl border border-[var(--fl-line)] bg-[var(--fl-ground)] p-4 text-xs text-[var(--fl-muted)]">{JSON.stringify(result, null, 2)}</pre>}
    </div>
  );
}

function Stat({ label, value, className }: { label: string; value: number; className: string }) {
  return <div className={`rounded-xl border p-4 ${className}`}><p className="text-xs font-semibold uppercase text-[var(--fl-muted)]">{label}</p><p className="mt-2 text-3xl font-semibold text-[var(--fl-text)]">{value}</p></div>;
}
