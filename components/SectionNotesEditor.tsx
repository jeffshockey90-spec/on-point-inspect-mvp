  ...
  if (editing) {
    return (
      <div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-3">
        <label className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-[var(--fl-warn-text)]">
        ...
        />
        {error && <p className="mt-2 text-xs font-bold text-[var(--fl-crit-text)]">{error}</p>}
  ...
  if (!notes.trim()) {
    return (
      <button
        ...
        className="rounded-lg border border-dashed border-[var(--fl-line)] px-3 py-2 text-xs font-semibold text-[var(--fl-muted)] transition hover:border-amber-400 hover:text-[var(--fl-warn-text)]"
      >
        + Add section note
      </button>
    );
  ...
  return (
    <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-3">
      <div className="flex items-start justify-between gap-3">
        <p className="whitespace-pre-wrap text-sm leading-6 text-[var(--fl-warn-text)]">{notes}</p>
        <button
          type="button"
          onClick={() => {
            setDraft(notes);
          ...
          className="shrink-0 rounded-lg border border-[var(--fl-line)] px-3 py-1.5 text-xs font-semibold text-[var(--fl-muted)] transition hover:border-amber-400 hover:text-[var(--fl-warn-text)]"
        >
          Edit
        </button>
      </div>
